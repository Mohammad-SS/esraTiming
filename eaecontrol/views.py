...
def sumbitEnter(request):
    ...
    user.update(isIn=True, lastStatusChangeTime=now)
...
def sumbitExit(request):
    ...
    user = models.Person.objects.filter(id=userId)
    now = datetime.datetime.now().time()
    user.update(isIn=False, lastStatusChangeTime=now)
...
def showTable(request):
    ...
    for user in users:
        recs = models.Timing.objects.filter(person=user , date=datetime.datetime.now().date())
        if recs.exists():
    ...
    context = {'users': users, 'date': jdatetime.datetime.now().strftime('%Y/%-m/%-d') ,'ver' : '1.1.0'}
    return render(request, 'eaecontrol/index.html', context)


def showPersonReport(request, id, m, y):
    if not request.user.has_perm('eaecontrol.add_person'):
        return redirect('/admin')
...
def showMounthlyReport(request, y, m):
    ...
    for user in users:
        ...
        while today <= persianMaxData.togregorian():
            standardEnterTime = datetime.datetime.combine(today, user.group.standardEnter)
            if today.weekday() == 3:
                standardExitTime = datetime.datetime.combine(today, user.group.standardExit) - datetime.timedelta(
                    hours=1)
            else:
                standardExitTime = datetime.datetime.combine(today, user.group.standardExit)
            print(today ,standardEnterTime, standardExitTime)
            balance = 0
            ...
            for todayWorkTime in todayWorkTimes:
                if todayWorkTime.type == 1:
                    balance += 1
                    if standardEnterTime > datetime.datetime.combine(today, todayWorkTime.time):
                        todayWorkTime.time = standardEnterTime
                ...
                if todayWorkTime.type == 0:
                    balance -= 1
                    if standardExitTime < datetime.datetime.combine(today, todayWorkTime.time):
                        todayWorkTime.time = standardExitTime
                    totalTodayWork = totalTodayWork + datetime.timedelta(hours=todayWorkTime.time.hour,
                                                                         minutes=todayWorkTime.time.minute,
                                                                         seconds=todayWorkTime.time.second)
            if not balance == 0:
            ...
            maximumWorkTime = standardExitTime - standardEnterTime
            if totalTodayWork > maximumWorkTime:
